<script lang="ts">
  ...
  let questions = $derived(bank.questions.filter(q => (!classId || q.classId === classId) && (!unitId || q.unitId === unitId) && (!sectionId || q.sectionId === sectionId) && (!query || `${q.body} ${q.tags.join(' ')} ${q.id}`.toLowerCase().includes(query))));
  let drafts = $derived(editor.session.drafts.filter(d => !query || `${d.fields.body} ${d.fields.tagInput}`.toLowerCase().includes(query)));
  function toggle(id: string) { selected = selected.includes(id) ? selected.filter(x => x !== id) : [...selected, id]; }
</script>
<div class="navigator">
  <input type="search" aria-label="Search questions and drafts" bind:value={search} oninput={() => limit = 80} placeholder="Search questions, tags, ID…" />
  <details><summary>Browse curriculum</summary><CurriculumPicker bind:classId bind:unitId bind:sectionId /></details>
  <div class="section-heading"><h3>Drafts <span>{editor.session.drafts.length}</span></h3><button onclick={() => selected = drafts.map(d => d.id)}>Select all</button></div>
  ...
  {/each}
  {#if !drafts.length}<p>No drafts. Create a question or open one below.</p>{/if}
